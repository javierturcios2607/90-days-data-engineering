...
class TransaccionVentas(BaseModel):
    ...
    monto_usd: float = Field(
        ..., ge=0.01, description="Monto de la venta en dólares. Debe ser mayor a cero."
    )
    ...
    metodo_pago: str = Field(
        default="NO_ESPECIFICADO", description="Método utilizado para pagar."
    )
    notas_adicionales: Optional[str] = Field(
        default=None, description="Comentarios opcionales de la venta."
    )
    ...
    def validar_fecha_no_futura(cls, valor_fecha: date) -> date:
        ...
        if valor_fecha > date.today():
            raise ValueError(
                "Data Quality Error: La fecha de la transacción no puede ser posterior al día de hoy."
            )
        return valor_fecha
