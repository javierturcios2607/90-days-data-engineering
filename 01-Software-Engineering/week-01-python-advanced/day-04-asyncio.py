        ...
        try:
            ...
            async with session.get(url, timeout=TIMEOUT_SECONDS) as response:
                if response.status == 200:
                    ...
                    logging.info(
                        f"Recurso {entity_id} procesado exitosamente: {data.get('name')}"
                    )
                    return data

                logging.warning(
                    f"Error en recurso {entity_id}: Status {response.status}"
                )
                return None
...
async def run_extraction_pipeline():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [fetch_entity_data(session, p_id, semaphore) for p_id in POKEMON_IDS]
