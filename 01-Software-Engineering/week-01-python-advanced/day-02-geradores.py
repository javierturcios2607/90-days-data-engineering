...
def timeit(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        fin = time.time()
        print(
            f"[Monitoreo] La función '{func.__name__}' tardó {fin - inicio:.4f} segundos."
        )
        return resultado
        ...
        def wrapper(*args, **kwargs):
            for intento in range(1, max_intentos + 1):
                try:
                    ...
                    print(
                        f"[Advertencia] Intento {intento}/{max_intentos} fallido: {e}"
                    )
                    if intento == max_intentos:
                        print(
                            f"[Error Crítico] La función '{func.__name__}' falló tras {max_intentos} intentos."
                        )
                        raise
                    time.sleep(delay)
