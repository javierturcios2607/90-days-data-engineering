...
def medir_tiempo_ejecucion(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        fin = time.time()
        print(
            f"[Telemetria] La funcion '{func.__name__}' tardo {fin - inicio:.4f} segundos."
        )
        return resultado
...
class AlmacenamientoLocalMixin:
    def guardar_json_local(
        self, datos: List[Dict[str, Any]], ruta_archivo: str
    ) -> None:
...
class IngestorGenerico(AlmacenamientoLocalMixin):
    ...
    def validar_en_flujo(
        self, datos_crudos: List[Dict[str, Any]]
        ...
        for registro in datos_crudos:
            try:
                ...
                print(
                    f"[Error de Calidad] Registro descartado. Motivo: {e.errors()[0]['msg']}"
                )
    ...
    async def ejecutar_pipeline(self, ruta_destino: str):
        ...
        print(
            f"[Exito] Pipeline finalizado. {len(datos_limpios)} registros procesados."
        )
